...
from (select annee, category, pname, dense_rank() over (partition by annee, category order by QTE_VENDUE desc) RANK
      from (select t.annee, p.category, p.pname, max(v.qte * v.pu) QTE_VENDUE
            from temps t, produits p, ventes v
...
select category,mois,jour,sum(qte_jour) 
from
  (
  select category, jour, mois, rank() over (partition by category,mois order by mois,jour ) rank_jour, qte_jour
  from
    (
    select p.category, t.jour , t.mois, sum(v.qte) qte_jour
    from ventes v, temps t, produits p
    where (v.tid=t.tid and v.pid=p.pid and t.annee=2010)
